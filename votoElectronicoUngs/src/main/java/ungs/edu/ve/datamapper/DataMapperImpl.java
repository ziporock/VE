...
public class DataMapperImpl<T> implements DataMapper<T> {
	...
	/*
	 * public static <T> Collection<T> map(ResultSet resultSet, DataMapper<T>
	 * mapper) throws SQLException { Collection<T> mappedObjects = new
	 * LinkedList<T>(); while(resultSet.next() ){
	 * mappedObjects.add(mapper.mapRow(resultSet)); } return mappedObjects; }
	 */
